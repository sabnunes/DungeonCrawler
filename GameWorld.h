...
public:
    ...
    PlayerMoveResult playerMove(const int x, const int y);	// move the player to a new position if it's valid and walkable, returns result
    ...
    void playerUseItem();			                        // consumes item and applies benefit
    ...
    // Occupancy functions
    bool isOccupiedByPlayer(const Position2D& position) const;  // returns if position is occupied by player
	bool isOccupiedByEnemy(const Position2D& position) const;   // returns if position is occupied by any enemy
	bool isOccupiedByItem(const Position2D& position) const;    // returns if position is occupied by any item
...
private:
    ...
    LevelDescription levelDescription;  // instance of current level definition
	GridMap map;                        // instance of the game map
    Player player;                      // player instance
    std::vector<Enemy> enemies;         // vector containing enemies in level based on level description
    std::vector<Item> items;            // vector containing items in level based on level description
